...
%token<anytype> SINGLE CMULF CMULS
%token<anytype> MINUS REM STRINGD CHAR CHARD FUNC VOID FOR WHILE BREAK CONTINUE RETURN
...
%type<anytype> condition inloop in_fun rtype fdata
%left OR
%left AND
...
func_declare:
FUNC IDEN LP fargs RP COLON rtype LCUR input RCUR {
    string name = $2.sval;
    if (var_type.find(name) != var_type.end()) {
...
in_fun:
RETURN fdata SEMIC {
    cout << "statement -> return : " << $2.sval << "\n";
}
;

fdata: {strcpy($$.sval, "void");}
|IDEN {strcpy($$.sval, $1.sval);}
|INTD {string tmp = to_string($1.ival);strcpy($$.sval, tmp.c_str());}
|DOUBLED {string tmp = to_string($1.dval);strcpy($$.sval, tmp.c_str());}
|STRINGD 
|BOOLD {string tmp = $1.bval ? "true":"false";strcpy($$.sval, tmp.c_str());}
|CHARD {string tmp; tmp += $1.cval;strcpy($$.sval, tmp.c_str());}
|expr {strcpy($$.sval, "expression");}
|condition {strcpy($$.sval, "expression");}
;
...
data:
|INTD|DOUBLED|BOOLD|STRINGD|CHARD|expr
|INTD COMMA data
|DOUBLED COMMA data
|BOOLD COMMA data
|STRINGD COMMA data
